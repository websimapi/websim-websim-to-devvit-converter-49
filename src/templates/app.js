...
router.get('/api/proxy/avatar/:username', async (req, res) => {
    // Sanitize username to prevent path traversal or errors
    const username = req.params.username ? req.params.username.trim() : 'guest';
    const DEFAULT_AVATAR = 'https://www.redditstatic.com/avatars/avatar_default_02_FF4500.png';

    // Immediate fallback for guest/anon
    if (!username || username.toLowerCase() === 'guest' || username.toLowerCase() === 'undefined' || username.toLowerCase() === 'null') {
        return res.redirect(DEFAULT_AVATAR);
    }
    ...
    try {
        ...
        res.redirect(url || DEFAULT_AVATAR);
    } catch (e) {
        // Fallback silently if user not found (404) or API error
