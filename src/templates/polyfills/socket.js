...
(function() {
    ...
    function sanitizeAvatar(url, username) {
        ...
        // If it's already a local proxy URL, return it as is
        if (url.includes('/api/proxy/avatar/')) {
            return url;
        }

        // If it's a WebSim URL or our old placeholder, rewrite to local proxy
        if (url.includes('images.websim.ai') || url.includes('images.websim.com') || url.includes('/_websim_avatar_/')) {
            const userPart = username || 'guest';
            return '/api/proxy/avatar/' + userPart;
        }
    ...
    class WebsimSocket {
        ...
        static updateIdentity(user) {
            ...
            if (user) {
                const safeUrl = sanitizeAvatar(user.avatar_url || user.avatarUrl, user.username);
                user.avatar_url = safeUrl;
                user.avatarUrl = safeUrl; // Dual-write for compatibility
            }
            window._currentUser = user;
            
            // Update the singleton socket instance's peer data immediately
            const inst = window.websimSocketInstance;
            if (inst && inst.peers[inst.clientId]) {
                inst.peers[inst.clientId].username = user.username;
                inst.peers[inst.clientId].avatarUrl = user.avatarUrl;
                // Force a presence update to propagate the new PFP to other players
                inst.updatePresence({ joined: true }); 
            }
        }
    }
    ...
    window.GenericDB = {
        ...
        getAdapter: (name) => ({
             ...
             create: (d) => {
                 ...
                 // Inject User Info if available (Support both snake_case and camelCase for legacy game logic)
                 if(window._currentUser) { 
                     r.username = window._currentUser.username; 
                     r.avatar_url = window._currentUser.avatar_url; 
                     r.avatarUrl = window._currentUser.avatar_url;
                     r.user_id = window._currentUser.id;
                 }
